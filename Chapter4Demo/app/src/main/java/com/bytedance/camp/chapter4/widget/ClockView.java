...
public class ClockView extends View {
    ...
    private void init() {
        ...
        needlePaint.setStrokeCap(Paint.Cap.BUTT);
        needlePaint.setStyle(Paint.Style.STROKE);
        // TODO 设置绘制时间数字的画笔: numberPaint
        numberPaint.setAntiAlias(true);
        numberPaint.setColor(Color.WHITE);
        numberPaint.setStrokeWidth(UNIT_LINE_WIDTH);
        numberPaint.setStrokeCap(Paint.Cap.BUTT);
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        int radians=second*360/60-90;
        second_x=(float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(radians)));
        second_y=(float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(radians)));
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,second_x,second_y,needlePaint);
        float min_x,min_y;
        radians/=60;
        radians+=minute*360/60-90;
        min_x=(float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(radians)));
        min_y=(float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(radians)));
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,min_x,min_y,needlePaint);
        float hour_x,hour_y;
        radians/=60;
        radians+=hour*360/12-90;
        hour_x=(float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(radians)));
        hour_y=(float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(radians)));
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,hour_x,hour_y,needlePaint);
